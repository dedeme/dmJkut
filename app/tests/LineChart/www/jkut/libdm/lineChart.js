...
export  function mkUnarySet(value, Line)  {sys.$params(arguments.length, 2);  return {value:value, Line:Line};};


export  function mkUnarySetExample()  {sys.$params(arguments.length, 0);  return mkUnarySet(0, mkLineExample());};
...
export  function mk(ExArea, InPadding, InAtts, ChartPadding, XAxis, YAxis, lang)  {sys.$params(arguments.length, 7);  return {
    ExArea:ExArea, InPadding:InPadding, InAtts:InAtts, ChartPadding:ChartPadding, XAxis:XAxis, YAxis:YAxis, lang:lang
  };};
...
export  function mkExample()  {sys.$params(arguments.length, 0);  return mk(
    mkAreaExample(),
    mkPaddingExample(),
    mkAreaAttsExample(),
    mkPadding(2, 1, 2, 1),
...
export  function mkWg(lC, Data)  {sys.$params(arguments.length, 2);
  ...
  const round =sys.$checkNull( Math.pow(10, Data.maxMinRound));
  if (sys.asBool(Max[0])) {
  ...
  for (let i  of sys.$forObject( iter.$range(0,arr.size(Data.Labels)))) {
    ...
    if (sys.asBool(!sys.asBool(Data.drawLabel(l, i)))) continue;
  ...
  cv.on("mousemove", function(ev)  {sys.$params(arguments.length, 1);
    ...
    for (let i  of sys.$forObject( iter.$range(0,arr.size(Data.SetValues)))) {
      ...
      for (let j  of sys.$forObject( iter.$range(0,arr.size(Vs)))) {
        const HotSet =sys.$checkNull( HotSetValues[i][j]);
    ...
    if (sys.asBool(sys.asBool(sys.$neq(UnarySetIx[0] ,  -1)) || sys.asBool(sys.$neq(SetIx[0] ,  -1)))) {
      ...
      if (sys.asBool(sys.$neq(SetIx[0] ,  -1))) {
        ...
        Tx1[0] =sys.$checkExists(Tx1[0],sys.$checkNull( "Line"));
      ...
      cv2
        ...
        .setStyle("left", "" + (ui.mouseX(ev) - Ms[0]) + "px")
